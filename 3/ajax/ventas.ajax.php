...
class AjaxVentas
{
    ...
    public function AjaxGenerarTicketVenta($nro_boleta)
    {
        ...
        $pdf->Cell(60, 6, 'TIENDA DE PRUEBA',0,0,'C');

        $pdf->Ln();
        $pdf->SetFont('Arial', '', 9);
        $pdf->Cell(60, 6, "RFC XAXX010101000  Toribio 444, San Francisco de Asis",0,0,'C');  

        $pdf->Ln();
        $pdf->Cell(60, 6, utf8_decode("Soledad de Graciano Sanchez - San Luis Potosi"),0,0,'C');     
        ...
        foreach ($productos as $pro) {
            ...
            $pdf->Cell(15, 4, "$ " . number_format($pro["precio_unitario_venta"], 2, ".", ","), 0, 0, 'R');

            $pdf->Cell(25, 5,  "$ " . number_format($pro["cantidad"] * $pro["precio_unitario_venta"], 2, ".", ","), 0, 0, "R");            
        ...
        $pdf->Cell(40, 4, "$ ". ROUND($total,2) - ROUND($total*0.18,2),0, 0, 'R');
        $pdf->Ln();
        ...
        $pdf->Cell(20, 4, "I.V.A.:" );
        $pdf->Cell(40, 4, "$ ". ROUND($total*0.18,2),0, 0, 'R');
        $pdf->Ln();
