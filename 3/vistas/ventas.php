        ...
        $('#lstProductosVenta tbody').on('click', '.btnIngresarPeso', function() {
            ...
            Swal.fire({
                ...
                if (result.value) {
                    ...
                    table.cell(idx, 5).data(cantidad + ' Metros').draw();

                    NuevoPrecio = ((parseFloat(data['cantidad']) * data['precio_venta_producto'].replace("M ", "")).toFixed(2));
                    NuevoPrecio = "$ " + NuevoPrecio;
                    ...
                    $.ajax({
                        ...
                        success: function(respuesta) {
                            ...
                            if (parseInt(respuesta['existe']) == 0) {
                                ...
                                NuevoPrecio = (parseFloat(1) * data['precio_venta_producto'].replaceAll("$ ", "")).toFixed(2);
                                NuevoPrecio = "$ " + NuevoPrecio;
                                ...
                                // ACTUALIZAR EL NUEVO PRECIO DEL ITEM DEL LISTADO DE VENTA
                                NuevoPrecio = (parseFloat(cantidad_actual) * data['precio_venta_producto'].replaceAll("$ ", "")).toFixed(2);
                                NuevoPrecio = "$ " + NuevoPrecio;
                                table.cell(index, 7).data(NuevoPrecio).draw();
                ...
                $.ajax({
                    ...
                    success: function(respuesta) {
                        ...
                        if (parseInt(respuesta['existe']) == 0) {
                            ...
                            // ACTUALIZAR EL NUEVO PRECIO DEL ITEM DEL LISTADO DE VENTA
                            NuevoPrecio = (parseFloat(cantidad_a_comprar) * data['precio_venta_producto'].replaceAll("$ ", "")).toFixed(2);
                            NuevoPrecio = "$ " + NuevoPrecio;
                            table.cell(index, 7).data(NuevoPrecio).draw();
        ...
        if (count > 0) {
            ...
            if ($("#iptEfectivoRecibido").val() > 0 && $("#iptEfectivoRecibido").val() != "") {
                ...
                table.rows().eq(0).each(function(index) {
                    ...
                    arr[index] = data['codigo_producto'] + "," + parseFloat($.parseHTML(data['cantidad'])[0]['value']) + "," + data['total'].replace("$ ", "");
