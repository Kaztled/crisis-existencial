...
class ProductosModelo
{
    ...
    static public function mdlCargaMasivaProductos($fileProductos)
    {
        ...
        $hojaProveedores = $documento->getsheetByName("Proveedores");
        $numeroFilasProveedores = $hojaProveedores->getHighestDataRow();
        ...
        $stmt = Conexion::conectar()->prepare("delete from Proveedores");
        $stmt->execute();
...
for ($i = 2; $i <= $numeroFilasProveedores; $i++) {
    $id = $hojaProveedores->getCellByColumnAndRow(1, $i)->getValue();
    $ruc = $hojaProveedores->getCellByColumnAndRow(2, $i)->getValue();
    $razon_social = $hojaProveedores->getCellByColumnAndRow(3, $i)->getValue();
    $direccion = $hojaProveedores->getCellByColumnAndRow(4, $i)->getValue();

    if (!empty($razon_social)) {
        $stmt = Conexion::conectar()->prepare("INSERT INTO Proveedores(id, ruc, razon_social, direccion)
                                                        VALUES (:id, :ruc, :razon_social, :direccion)");
        ...
        if ($stmt->execute()) {
            $ProveedoresRegistrados = $ProveedoresRegistrados + 1;
        } else {
            $ProveedoresRegistrados = 0;
        }
    }
        ...
        $respuesta["totalProveedores"] = $ProveedoresRegistrados;
